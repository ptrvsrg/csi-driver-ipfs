...
current_year=$(date +%Y)
find . -type d \( -name "deploy/kubernetes" -o -name "charts" -o -name "node_modules" -o -name "vendor" \) -prune -o -type f \( -name "*.go" -o -name "*.sh" -o -name "*.yaml" -o -name "*.yml" \) -print | while read -r file; do
  case "$file" in
  **/mock_*.go)
